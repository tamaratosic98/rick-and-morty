import { Button, Flex, Form, FormProps, Image, Input, Select, Spin } from 'antd';
import { useMemo } from 'react';
import { CharacterFormMode } from '../../../../modules/character/character.constants';
import { useCharacter, useOptimisticUpdateCharacter } from '../../../../modules/character/character.queries';
...
export const CharacterForm = ({
  ...
  const onFinish: FormProps<Character & { locationName: string }>['onFinish'] = (
    values: Character & { locationName: string }
  ) => {
    const newLocation = {
  ...
  return (
    <>
      {!isLoading ? (
        <Form name="basic" onFinish={onFinish} layout="horizontal" autoComplete="off" size="large">
          <Flex justify="center" className="pb-10">
            <Image src={character?.image} preview={false} className="rounded-md" width={150} />
          </Flex>
          ...
          <Form.Item name="species" label="Species" initialValue={character?.species}>
          ...
          <Form.Item name="status" label="Status" initialValue={character?.status}>
            <Select disabled={isDisabled}>
          ...
          <Form.Item name="gender" label="Gender" initialValue={character?.gender}>
            <Select disabled={isDisabled}>
          ...
          <Form.Item name="locationName" label="Location Name" initialValue={character?.location?.name}>
            <Input disabled={isDisabled} />
          </Form.Item>
